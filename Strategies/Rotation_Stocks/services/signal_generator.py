"""
Refactored Stock Signal Generator
Implements new 3-step workflow:
1. Fetch strategy details from stock_saved_strategy
2. Generate signals using 52-week high/low logic
3. Save to stock_signal table
"""
import sys
import os
import pandas as pd
...
class LiveStockSignalGenerator:
    """
    Live Stock Signal Generator - Refactored
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(os.path.join(log_dir, 'stock_signals.log'), encoding='utf-8'),
    ...
    def fetch_running_strategies(self, user_id: str = None) -> List[Dict[str, Any]]:
        """
        Step 1: Fetch running strategies from stock_saved_strategy table
        ...
        try:
            ...
            session = get_session()
            try:
                # Query stock_saved_strategy WHERE status='running'
                query = session.query(StockSavedStrategy).filter(
                    StockSavedStrategy.status == 'running'
                )
                
                if user_id:
                    query = query.filter(StockSavedStrategy.user_id == user_id)
    ...
    def load_stock_data(self, symbol: str, days_back: int = 365) -> pd.DataFrame:
    ...
    def calculate_52week_metrics(self, tickers: List[str]) -> pd.DataFrame:
        """Calculate 52-week high/low metrics for Stocks"""
        try:
            all_metrics = []
    ...
    def generate_signals(self, metrics_df: pd.DataFrame, accumulation_weeks: int = 0) -> List[Dict[str, Any]]:
        ...
        BUY: Top 1 Stock closest to 52-week LOW
        SELL: Top 3 Stocks closest to 52-week HIGH
        """
        try:
            signals = []
    ...
    def save_signals(self, signals: List[Dict[str, Any]], strategy_details: Dict[str, Any]):
        ...
        try:
            from Databases.app_data_db_connection import get_session
            from Databases.strategy_models import StockSignal
            ...
            try:
                for signal in signals:
                    ...
                    session.add(stock_signal)
                
                session.commit()
                self.logger.info(f"Saved {len(signals)} signals to stock_signal table for strategy: {strategy_details['strategy_name']}")
    ...
    def run_weekly_signal_generation(self, user_id: str = None) -> Dict[str, Any]:
        ...
        try:
            print("\n" + "="*80)
            print("STOCK SIGNAL GENERATION - 3-STEP WORKFLOW")
            if user_id:
                print(f"Target User: {user_id}")
            print("="*80 + "\n")
            
            # STEP 1: Fetch running strategies
            print("STEP 1: Fetching running strategies from stock_saved_strategy...")
            print("-" * 80)
            ...
            for strategy in strategies:
                ...
                print(f"Stocks: {', '.join(strategy['tickers'][:5])}{'...' if len(strategy['tickers']) > 5 else ''}")
                print()
                ...
                # STEP 3: Save signals
                print("STEP 3: Saving signals to stock_signal table...")
                print("-" * 80)
